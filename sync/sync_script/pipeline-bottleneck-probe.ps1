...
<#
.SYNOPSIS
    Pipeline bottleneck probe v0.1 (Argus-B 自律執行、2026-05-01 Yas メーター計測提案契機)
...
$overallAlert = @($stage1Alert, $stage2Alert, $queueAlert) + ($taskHealth | ForEach-Object { $_.Alert }) |
...
$snapshot = [ordered]@{
    ...
    generator = "pipeline-bottleneck-probe.ps1 v0.1"
