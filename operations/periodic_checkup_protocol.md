# 三者定期検診プロトコル v0.6 — アップデート前後の数値変化チェック + 成長軌跡可視化

**配置先**: `operations/periodic_checkup_protocol.md`
**起草日**: 2026-04-28(Day 130 末)、**v0.6 追記**: 2026-05-04（`factory_code_health`）
...
#### 5-B-1. 工場コード健全性メトリクス（`factory_code_health`、第 125 次発令）

工場長は **週次以上**の定期検診レポート（既存の工場系レポート出力パイプラインに統合）へ、次を **固定ブロック**として載せる。司令官 α が `commander#sync/completion_reports/` へ着地させ、監督官は `sync-factory-pipeline.ps1` ミラー等で間接観測する。

| フィールド | 必須 |
|---|---|
| `captured_at` / `repo_head_sha` | ✅ |
| `test_ts_files` / `prod_ts_tsx_files` | ✅ |
| `test_lines_total` / `prod_lines_total` / `test_to_prod_line_ratio` | ✅（`node_modules`・`dist` 除外で定義） |
| `vitest_duration_sec` | 取得可能なら ✅、不能時は null + 理由 |
| `health_flag`（🟢🟡🔴） | ✅（閾値は工場長が ADR/ticket で宣言、未確定時は暫定表をレポートに併記） |
| `interpretation_note` | 任意（チケット起因のテスト増など 1 行） |

**目的**: AI 駆動開発にありがちな **テストだけ肥大・本番空洞化** を早期検知するための **機械可読な先行指標**。ルーブリックの `axis_3_factory_progress_observation` と併読する。
...
#### 5-C-1. 工場コード健全性の監督官チェック（ミラー二次確認）

監督官は **週次自己採点**または **三者検診**のたびに次を確認する（詳細手順は `operations/supervisor_self_checkup_prompt.md` §2-D 連動）。

1. 司令官経由で着地した最新 `completion_reports`（または同等 JSON）に **`factory_code_health` ブロック**が存在するか。  
2. `test_to_prod_line_ratio` が **前回比で異常上昇**していないか（暫定: 単回 +0.10 以上 = 黄扱いで記録）。  
3. 欠落時は **EVT 候補**（経路滞留ではなく「レポート契約不履行」）として `notes` に記し、司令官 α へ **再着地要請**（工場長経由）。
...
### 6-B-1. 出典列(§7-H 連動、手動 growth_report 推奨)

5 時点スコア比較表に **出典** 列を追加し、各セルに `self_score`(スナップショット自己採点) / `mechanical_recheck`(`verify-role-snapshot-vs-rubric.ps1` 出力) / `supervisor_note`(監督官コメント)のいずれかを明記する。機械再検査未実施の数値を「確定診断」として書かない。
...
### 7-H. 監督官二次検証レイヤー(ガチ層、v0.5 新設、2026-05-04 Yasu 承認)

#### 7-H-1. 起案契機

司令官 `snapshot_commander_*.json` は **自己採点 + ナラティブ根拠**を含む。監督官が JSON を要約するだけでは **再現可能な検証**にならない(叙述の信頼移転リスク)。本節は `rubrics/role_execution_rubric.yaml` v0.2 に明示された **機械算出 formula 対象**について、司令官リポジトリ実体から **決定的に再計算**し、スナップショット同一項目と突合する **二次検証**を定期検診に組み込む。

#### 7-H-2. 目的と非目的

| 種別 | 内容 |
|---|---|
| **目的** | (i) 機械スコア vs スナップショット self score の差分検出、(ii) 証拠パス(mtime 入力ファイル)のロック、(iii) 三者検診表への **出典ラベル**(自己採点 / 機械再検査 / 監督官注記)付与 |
| **非目的** | 主観軸(SITREP 鮮度、circular_close 等)の自動採点代替、司令官採点の全面上書き、凍結装置(`pipeline-bottleneck-probe.ps1` 等)の無断改修 |

#### 7-H-3. 入力

| # | 入力 | 解決順序 |
|---|---|---|
| 1 | ミラー済み `sync/checkup-scores/role_execution/snapshot_commander_*.json` | 比較対象の self score |
| 2 | 司令官リポジトリ root | 環境変数 `RX_COMMANDER_REPO` → `sync/sync_script/checkup-verification.config.json` の `commanderRepo` → 手動 `-CommanderRoot` |
| 3 | Rubric | 本リポ `rubrics/role_execution_rubric.yaml`(参照仕様、v0.1 スクリプトは数式を内蔵) |

**規律**: `RX_COMMANDER_REPO` 未設定かつ config 欠落で commander パスが解決できない場合、検証結果の `overall_status` は **`aligned` とみなさない** → `unavailable_path` とし、ナラティブのみの「健全」宣言を禁ずる。

#### 7-H-4. 装置(物理スクリプト)

| 装置 | 配置 | 動作 |
|---|---|---|
| `verify-role-snapshot-vs-rubric.ps1` v0.1 | `sync/sync_script/` | v0.1 スコープ: `advance_map_freshness` + `ticket_issuance_velocity` の機械再計算 + 差分 + `drift_warning` / `unavailable_path` / `aligned` |
| `checkup-verification.config.json` | `sync/sync_script/` | `commanderRepo` + `driftScoreDeltaThreshold`(既定 15) |

**CI / 手動**: 既定 **exit 0**(警告はコンソール + JSON)。厳格モードは `-FailOnDrift`(exit 2、`drift_warning` 時)。

#### 7-H-5. 出力(`supervisor_verification` ブロック)

並列ファイル推奨: `snapshot_commander_{...}_verification.json`(スナップショットと同ディレクトリ、`-WriteSidecar`)。必須フィールド:

- `captured_at_utc`
- `mechanical_scores` / `snapshot_scores` / `delta_by_component`
- `drift_threshold` / `drift_components[]`
- `path_status`(キー別 `unavailable_path` | `missing_file` | `mechanical_unavailable` | `snapshot_missing`)
- `overall_status`: `aligned` | `drift_warning` | `unavailable_path`
- `evidence_paths[]`

**閾値**: コンポーネント差分 **≥ `driftScoreDeltaThreshold`(既定 15)** または **緑/黄/赤帯(80/60 境界)を跨ぐ**場合 `drift_warning` に含める。

#### 7-H-6. 三者検診表への合成(09:00 JST 三者検診、§7-F-3 連動)

同一検診イベントで、少なくとも次を **1 表に併記**する:

| 列セット | 内容 |
|---|---|
| A | 司令官 `role_total` + 4 軸(ミラー JSON) |
| B | `factory_pipeline/snapshot_*.json` の最新 1 件の `captured_at` + 主要指標(例: tickets 24h) |
| C | 監督官 `snapshot_supervisor_*.json`(存在すれば同 checkpoint 近傍) |
| D | **工場** 最新定期検診レポート内 `factory_code_health`（`test_to_prod_line_ratio` + `health_flag` + 前回比 1 行）— 未着地 = 🔴 欠落 |
| **出典** | 各数値に `self_score` / `mechanical_recheck` / `supervisor_note` のいずれかを付す(司令官軸は A が self、7-H 機械結果は `mechanical_recheck`)。D 列は `factory_report` / `supervisor_note` |

#### 7-H-7. drift 時の次アクション(監督官)

| 状態 | 次アクション |
|---|---|
| `drift_warning` | (i) 司令官 α へ再採点 or 根拠追補要請、(ii) ミラー時刻ズレ・別 branch 参照の疑いを排除、(iii) 意図的オーバーライドならスナップショット `notes` に理由 1 段落 |
| `unavailable_path` | `RX_COMMANDER_REPO` / config 修正 → 検証再実行。再実行まで当該コンポーネントは **検証不能**として growth_report に記す |
| `aligned` | 記録のみ。次回検診まで追加作業不要 |

#### 7-H-8. v0.2 拡張候補(未着手)

- `verdict_distribution` 等、inbox パスが commander 側に依存する formula の同型実装
- Rubric コンポーネント名と JSON キーの **正規化マッピング表**(命名不一致解消)

#### 7-H-9. `factory_code_health` の監督官機械補助（任意、Phase B 以降）

- `sync-factory-pipeline.ps1` または後継が、ミラー JSON に **`factory_code_health_last_seen`**（`captured_at` + ratio + flag）を **抽出コピー**できれば、三者表 D 列の手負荷を下げる。
- 抽出不能時は §5-C-1 の目視チェックが SSOT。
...
- `sync/sync_script/verify-role-snapshot-vs-rubric.ps1` v0.1 + `checkup-verification.config.json`(§7-H 監督官二次検証レイヤー)
...
- **v0.5**(2026-05-04 / Day 132、Yasu 承認): §7-H 監督官二次検証レイヤー(ガチ層)新設。`verify-role-snapshot-vs-rubric.ps1` v0.1 + `checkup-verification.config.json` 物理装置化。機械再計算対象(初回)= advance_map_freshness + ticket_issuance_velocity。出力 `supervisor_verification` / sidecar JSON、三者検診表への出典列、drift 時次アクション。凍結装置 `pipeline-bottleneck-probe.ps1` には手を入れず独立実行。起案根拠 = `staging/periodic_checkup_supervisor_gachi_layer_plan_v0.1.md` 採択完遂。
- **v0.6**(2026-05-04 / Day 137 想定週次運用): §5-B-1 工場コード健全性 `factory_code_health`（第 125 次発令）+ §5-C-1 監督官ミラー二次確認 + §7-H-6 三者表 D 列 + §7-H-9 任意機械補助。工場長実装・司令官着地の後に運用開始。
